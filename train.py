...
'''
class Trainer(object):训练类 
包括训练的参数（batchs ，epoch，优化器、学习率）、加载经过图像处理的
定义函数包括 -加载预训练权重、保存训练模型。训练
'''
...
class Trainer(object):
    def __init__(self, weight_path, resume, gpu_id):
        ...
        self.multi_scale_train = cfg.TRAIN["MULTI_SCALE_TRAIN"]  # 多尺度训练
        ...
        self.optimizer = optim.SGD(self.yolov3.parameters(), lr=cfg.TRAIN["LR_INIT"],
                                   momentum=cfg.TRAIN["MOMENTUM"], weight_decay=cfg.TRAIN["WEIGHT_DECAY"])  # 优化器
        ...
        self.criterion = YoloV3Loss(anchors=cfg.MODEL["ANCHORS"], strides=cfg.MODEL["STRIDES"],
                                    iou_threshold_loss=cfg.TRAIN["IOU_THRESHOLD_LOSS"])  # loss的计算

        self.__load_model_weights(weight_path, resume)  # 加载权重
    ...
    def train(self):
        ...
        for epoch in range(self.start_epoch, self.epochs):
            ...
            for i, (imgs, label_sbbox, label_mbbox, label_lbbox, sbboxes, mbboxes, lbboxes) in enumerate(
                ...
                # writer2.add_scalar("loss", loss.item(), global_step=epoch * len(self.train_dataloader) + i,
                #                    walltime=None)
