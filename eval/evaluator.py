...
class Evaluator(object):
    ...
    def APs_voc(self, multi_test=False, flip_test=False):
        ...
        for img_ind in tqdm(img_inds):
            img_path = os.path.join(self.val_data_path, 'images', img_ind+'.jpg')
    ...
    def get_bbox(self, img, multi_test=True, flip_test=True):
        if multi_test:
        ...
        # a = torch.from_numpy(bboxes[:, :4])
        # b = torch.from_numpy(bboxes[:, 4])
        # c=torchvision.ops.nms(a, b, self.nms_thresh).numpy()
        # #print(bboxes[c,:])
        # bboxes= bboxes[c,:]
        # print(bboxes.dtype)
        # print(bboxes)
        #bboxes = nms_all(bboxes, self.conf_thresh, self.nms_thresh)
        #print(bboxes)
        return bboxes
    ...
    def __calc_APs(self, iou_thresh=0.5, use_07_metric=False):
        ...
        annopath = r"F:\deeplearn\yolov3-master\data\Annotations\{:s}.xml"
        # imagesetfile = os.path.join(self.val_data_path,  'ImageSets', 'val.txt')
        imagesetfile = os.path.join(self.val_data_path, 'ImageSets', 'test.txt')
        APs = {}
...
#a = Evaluator(Yolov3().to(gpu.select_device(0))).cal_APs()
